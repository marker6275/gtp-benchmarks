(module dealer racket/base
  (#%module-begin
   ...
   (require (only-in "basics.rkt" FACE FIVE STACKS SIXTYSIX HAND MIN-BULL MAX-BULL configuration))
   ...
   (define dealer%
     (class object%
       ...
       (field
        (internal%
         (class object%
           ...
           (define/public (add-score n) (send this set-field:my-bulls (+ n (send this get-field:my-bulls))))
        ...
        (internals (for/list ((p (in-list (send this get-field:players)))) (new internal% (player p)))))
       ...
       (define/public
        ...
        (when (> (+ (* n HAND) STACKS) FACE) (error 'play-game "cannot play with ~a players; more cards needed" n))
        (let play-game ((i 1)) (play-round shuffle faces) (if (any-player-done?) (present-results i) (play-game (+ i 1)))))
       (define/public
        (present-results i)
        (define sorted (sort (send this get-field:internals) < #:key (lambda (i) (send i bulls))))
        `((after-round ,i) ,(for/list ((p (in-list sorted))) `(,(send p name) ,(send p bulls)))))
       (define/public (any-player-done?) (for/or ((p (in-list (send this get-field:internals)))) (> (send p bulls) SIXTYSIX)))
       (define/public
       ...
       (define/private (deal-cards card-pool) (for ((p (in-list (send this get-field:internals)))) (send p start-round (send card-pool draw-hand))))
       (define/private
        (play-turn deck)
        (define played-cards (for/list ((p (in-list (send this get-field:internals)))) (list p (send p start-turn deck))))
        (define sorted-played-cards (sort played-cards < #:key (lambda (x) (card-face (second x)))))
         ...
         (cond
          ((send deck larger-than-some-top-of-stacks? card)
           ...
           (cond
            ...
            ((= (length closest-fit-stack) FIVE) (define bulls (send deck replace closest-fit-stack card)) (send player add-score bulls))))
          (else (define chosen-stack (send player choose deck)) (define bulls (send deck replace chosen-stack card)) (send player add-score bulls)))))))))

...
(define/contract new-label
  any/c
  gensym)

;(: make-ref (-> Var Exp))
(define/contract (make-ref var)
  (configurable-ctc
   [max (->i ([var Var?])
             [result (var)
                     (and/c Ref-type/c
...
(define/contract (make-lambda formals call)
  (configurable-ctc
   [max (->i ([formals (listof Var?)]
              [call Exp-type/c])
             [result (formals call)
                     (and/c Lam-type/c
                            (Lam/c symbol?
...
(define/contract (make-call fun . args)
  (configurable-ctc
...
(define/contract (make-let var exp call)
  (configurable-ctc
   [max (->i ([var Var?]
              [exp Exp-type/c]
              [call Call-type/c])

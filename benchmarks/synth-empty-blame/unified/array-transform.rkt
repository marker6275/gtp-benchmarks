(module array-transform typed/racket/shallow
  (#%module-begin
   (require "../../../utilities/require-typed-check-provide-transient.rkt" racket/vector (only-in racket/fixnum fx+) "typed-data.rkt")
   ...
   (require/typed/check "array-broadcast.rkt" (array-broadcast (-> Array Indexes Array)) (array-shape-broadcast (-> (Listof Indexes) Indexes)))
   (require/typed/check
    "array-utils.rkt"
   ...
   (: array-broadcast-for-append (-> (Listof Array) Integer (values (Listof Array) (Listof Integer))))
   (define (array-broadcast-for-append arrs k)
     ...
     (cond
      ...
      ((or (< k 0) (>= k dims)) (raise-argument-error 'array-append* (format "Index < ~a" dims) k))
      (else
       (let* ((dss (map (λ ((ds : Indexes)) (define dms (vector-length ds)) (vector-append ((inst make-vector Integer) (- dims dms) 1) ds)) dss))
         ...
         (define new-arrs (map (λ ((arr : Array) (ds : Indexes)) (array-broadcast arr ds)) arrs dss))
         (values new-arrs dks)))))
        ...
        (else
         ...
         (let arrs-loop ((arrs arrs) (dks dks) (jk 0))
           (unless (null? arrs)
             ...
             (define dk (car dks))
             (let i-loop ((i 0) (jk jk)) (cond ((< i dk) (vector-set! old-procs jk proc) (vector-set! old-jks jk i) (i-loop (+ i 1) (fx+ jk 1))) (else (arrs-loop (cdr arrs) (cdr dks) jk))))))
         (: arr* Array)
         (define arr*
           (unsafe-build-array
            new-ds
            (λ ((js : Indexes)) (define jk (vector-ref js k)) (vector-set! js k (vector-ref old-jks jk)) (define v ((vector-ref old-procs jk) js)) (vector-set! js k jk) v)))

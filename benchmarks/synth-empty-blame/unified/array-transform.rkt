(module array-transform racket/base
  (#%module-begin
     ...
     (cond
      ...
      (else
       (let* ((dss
         ...
         (define new-arrs (map (λ (arr ds) (array-broadcast arr ds)) arrs dss))
         (values new-arrs dks)))))
   (define (array-append* arrs (k 0))
     ...
     (let-values (((arrs dks) (array-broadcast-for-append arrs k)))
       ...
       (cond
        ((not (index? new-dk))
         (error 'array-append* "resulting axis is too large (not an Index)"))
        (else

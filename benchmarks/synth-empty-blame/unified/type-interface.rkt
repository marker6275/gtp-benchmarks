(module mutation-adapter typed/racket/shallow
  (#%module-begin
   (module contracted racket
     ...
     (provide (except-out (all-from-out "original-type-interface.rkt") unsafe-vector->array))
     (begin
       (define unsafe-vector->array1166666
         (contract (swap-> #t 0 1) unsafe-vector->array #f #f))
       (provide (rename-out (unsafe-vector->array1166666 unsafe-vector->array)))))

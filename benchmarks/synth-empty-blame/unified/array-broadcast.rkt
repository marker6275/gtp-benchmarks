(module array-broadcast typed/racket/shallow
  (#%module-begin
   ...
   (require/typed/check "array-utils.rkt" (make-thread-local-indexes (-> Integer (-> Indexes))))
   ...
   (define (shift-stretch-axes arr new-ds)
     ...
     (define shift
       (let ((shift (- new-dims old-dims))) (cond ((index? shift) shift) (else (error 'array-broadcast "cannot broadcast to a lower-dimensional shape; given ~e and ~e" arr new-ds)))))
   ...
   (define (array-broadcast arr ds)
     (cond
      ...
      (else (define new-arr (shift-stretch-axes arr ds)) (if (or (array-strict? arr) (fx<= (array-size new-arr) (array-size arr))) new-arr (begin (array-default-strict! new-arr) new-arr)))))
   ...
   (define (shape-permissive-broadcast ds1 ds2 dims fail)
     ...
     (let loop ((k 0))
       (cond
        ((< k dims) (define dk1 (vector-ref ds1 k)) (define dk2 (vector-ref ds2 k)) (vector-set! new-ds k (cond ((or (= dk1 0) (= dk2 0)) (fail)) (else (fxmax dk1 dk2)))) (loop (+ k 1)))
        (else new-ds))))
     ...
     (let loop ((k 0))
       (cond
        ((< k dims)
         ...
         (vector-set! new-ds k (cond ((= dk1 dk2) dk1) ((and (= dk1 1) (> dk2 0)) dk2) ((and (= dk2 1) (> dk1 0)) dk1) (else (fail))))
         (loop (+ k 1)))
        (else new-ds))))
   ...
   (define (shape-broadcast2 ds1 ds2 fail broadcasting)
     (cond
      ...
      (else
       ...
       (let-values (((ds1 ds2 dims) (cond ((> n 0) (values (shape-insert-axes ds1 n) ds2 dims2)) ((< n 0) (values ds1 (shape-insert-axes ds2 (- n)) dims1)) (else (values ds1 ds2 dims1)))))
         (if (eq? broadcasting 'permissive) (shape-permissive-broadcast ds1 ds2 dims fail) (shape-normal-broadcast ds1 ds2 dims fail))))))
   ...
   (define (array-shape-broadcast dss (broadcasting (array-broadcasting)))
     (define (fail) (error 'array-shape-broadcast "incompatible array shapes (array-broadcasting ~v): ~a" broadcasting (string-join (map (λ (ds) (format "~e" ds)) dss) ", ")))
     (cond ((empty? dss) #()) (else (for/fold ((new-ds (first dss))) ((ds (in-list (rest dss)))) (shape-broadcast2 new-ds ds fail broadcasting)))))))

(module array-broadcast typed/racket/shallow
  (#%module-begin
   ...
   (provide array-broadcasting array-broadcast array-shape-broadcast)
   ...
   (define (shape-insert-axes ds n) (vector-append ((inst make-vector Integer) n 1) ds))
     ...
     (let loop ((k 0))
       (cond
        ((< k dims)
         ...
         (vector-set! new-ds k (cond ((or (= dk1 0) (= dk2 0)) (fail)) (else (fxmax dk1 dk2))))
         (loop (+ k 1)))
        (else new-ds))))
   ...
   (:
    ...
    (case-> ((Listof Indexes) -> Indexes) ((Listof Indexes) (U #f #t 'permissive) -> Indexes)))

  ...
  (#%module-begin
        ...
        (case dims
          ...
          ((2)
           ...
           (let j0-loop ((j0 0) (j 0))
             (when (< j0 d0)
               (vector-set! js 0 j0)
               (let j1-loop ((j1 0) (j j))
                 (cond
                  ((< j1 d1) (vector-set! js 1 j1) (f js j) (j1-loop (+ j1 1) (fx+ j 1)))
                  (else (j0-loop (+ j0 1) j)))))))
          (else
           (let i-loop ((i 0) (j 0))
             (cond
              ((< i dims)
               ...
               (let ji-loop ((ji 0) (j j))
                 (cond
                  ((< ji di) (vector-set! js i ji) (ji-loop (+ ji 1) (i-loop (+ i 1) j)))
                  (else j))))
              (else (f js j) (fx+ j 1))))
           (void)))))))
   (define-syntax-rule
           ...
           (let i-loop ((i 0))
             (cond
              ((< i dims)
               (define di (vector-ref ds i))
               (let ji-loop ((ji 0))
                 (when (< ji di) (vector-set! js i ji) (i-loop (+ i 1)) (ji-loop (+ ji 1)))))
              (else (f js))))))))))
   (define-syntax-rule
      ...
      (cond
       ...
       (else
        (case dims
          ((1)
           (define d0 (vector-ref ds 0))
           (let j0-loop ((j0 0)) (when (< j0 d0) (f j0) (j0-loop (+ j0 1)))))
          ((2)
           (define d0 (vector-ref ds 0))
           (define d1 (vector-ref ds 1))
           (let j0-loop ((j0 0) (j 0))
   ...
   (define-syntax-rule
    ...
    (let* ((ds ds-expr) (dims (vector-length ds)))
      ...
      (define size
        (let loop ((k 0) (size 1))
          (cond ((< k dims) (loop (+ k 1) (fx* size (vector-ref ds k)))) (else size))))
      (cond
       ((= size 0) (vector))
       (else
   ...
   (define-syntax-rule
    ...
    (lambda (js) (ref (unsafe-array-index->value-index ds js))))
     ...
     (let ((f (box f)))
       ...
       (define (strict!)
         (let* ((old-f (unbox f))
           ...
           (set-box! f (lambda (js) (vector-ref vs (unsafe-array-index->value-index ds js))))))
   ...
   (define (build-array ds proc)
     (let ((ds
            (check-array-shape
             ds
             (lambda () (raise-argument-error 'build-array "(Vectorof Index)" 0 ds proc)))))
   ...
   (define (build-simple-array ds proc)
     (let ((ds
            (check-array-shape
             ds
             (lambda ()
               (raise-argument-error 'build-simple-array "(Vectorof Index)" 0 ds proc)))))
   ...
   (define-syntax-rule
    ...
    (lambda (js v) (set! (unsafe-array-index->value-index ds js) v)))
   (define (make-array ds v)
     (let ((ds
            (check-array-shape
             ds
             (λ () (raise-argument-error 'make-array "(Vectorof Index)" 0 ds v)))))

(module array-utils typed/racket/shallow
  (#%module-begin
   ...
   (begin-encourage-inline
    ...
    (define (vector->supertype-vector js)
      ...
      (cond
       ((= dims 0) (vector))
       (else
        (define new-js : (Vectorof (U A B)) (make-vector dims (vector-ref js 0)))
        (let loop ((i 1))
          (cond
    ...
    (define (array-shape-size ds)
      ...
      (let loop ((i 0) (n 1))
        (cond ((< i dims) (define d (vector-ref ds i)) (loop (+ i 1) (* n d))) (else n))))
    ...
    (define (check-array-shape-size name ds)
      ...
      (cond
       ((index? size) size)
       (else
        (error name "array size ~e (for shape ~e) is too large (is not an Index)" size ds))))
      ...
      (let loop ((i 0))
        (cond
         ((< i dims)
          ...
          (cond ((index? di) (vector-set! new-ds i di) (loop (+ i 1))) (else (fail))))
         (else new-ds))))
     ...
     (let loop ((i 0) (j 0))
       (cond
        ((< i dims)
         ...
         (define ji (vector-ref js i))
         (cond
          ((and (exact-integer? ji) (<= 0 ji) (< ji di)) (loop (+ i 1) (fx+ ji (fx* di j))))
   ...
   (define (unsafe-vector-remove vec k)
     ...
     (cond
      ...
      (else
       ...
       (let loop ((i 0))
         (when (< i k) (vector-set! new-vec i (vector-ref vec i)) (loop (+ i 1))))
       (let loop ((i k))
         (cond
          ((< i n-1) (vector-set! new-vec i (vector-ref vec (+ i 1))) (loop (+ i 1)))
          (else new-vec))))))
   ...
   (define (unsafe-vector-insert vec k v)
     ...
     (let loop ((i k))
       (when (< i n)
         (let ((i+1 (+ i 1))) (vector-set! dst-vec i+1 (vector-ref vec i)) (loop i+1))))
     dst-vec)
     ...
     (let ((val : (Thread-Cellof (U #f Indexes)) (make-thread-cell #f)))
       (λ ()
         (or (thread-cell-ref val)
             (let ((v : Indexes (make-vector dims 0))) (thread-cell-set! val v) v)))))

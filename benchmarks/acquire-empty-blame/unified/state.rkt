(module state typed/racket/shallow
  (#%module-begin
   (provide score?
            (struct-out player)
            player0
   ...
   (require "../../../utilities/require-typed-check-provide-transient.rkt" "../base/types.rkt" "board-adapted.rkt")
   (require/typed/check
    "basics.rkt"
   ...
   (require/typed/check "auxiliaries.rkt" (aux:partition (All (A B) (-> (Listof A) (-> A Real) (-> A B) (Listof (Listof B))))) (distinct (-> (Listof Any) Boolean)))
   ...
   (struct player ((name : String) (tiles : (Listof Tile)) (money : Cash) (shares : Shares) (external : (Option (Instance Player%)))) #:transparent)
   (: player0 (-> String Tile Tile Tile Tile Tile Tile (Instance Player%) Player))
   (define (player0 n t1 t2 t3 t4 t5 t6 x) (player n (list t1 t2 t3 t4 t5 t6) CASH0 player-shares0 x))
   (: ext:*create-player (-> String Cash Shares (Listof Tile) Player))
   (define (ext:*create-player name cash shares tiles)
     (unless (distinct tiles) (error '*create-player (format "Precondition: distinct tiles ~a" tiles)))
     (unless (<= STARTER-TILES# (length tiles)) (error '*create-player (format "Precondition: <=~a tiles in ~a" STARTER-TILES# tiles)))
     (*create-player name cash shares tiles))
   (: *create-player (-> String Cash Shares (Listof Tile) Player))
   (define (*create-player name cash shares tiles) (player name tiles cash shares #f))
   (: player-tile- (-> Player Tile Player))
   (define (player-tile- p t) (struct-copy player p (tiles (remove t (player-tiles p)))))
   (: player-tile+ (-> Player Tile Player))
   (define (player-tile+ p t) (struct-copy player p (tiles (cons t (player-tiles p)))))
   (: player-shares++ (-> Player Hotel * Player))
   (define (player-shares++ p . h) (if (empty? h) p (struct-copy player p (shares (for/fold : Shares ((s : Shares (player-shares p))) ((h : Hotel (in-list h))) (shares++ s h))))))
   (: player-buy-shares (-> Player (Listof Hotel) Board Player))
   (define (player-buy-shares p0 sh board)
     (define amount (for/sum : Cash ((h : Hotel sh)) (or (price-per-share h (size-of-hotel board h)) 0)))
   ...
   (struct state ((board : Board) (players : (Listof Player)) (tiles : (Listof Tile)) (hotels : (Listof Hotel)) (shares : Shares) (bad : (Listof Player))) #:transparent)
   (define-type State state)
   (: ext:state0 (-> Player * State))
   (define (ext:state0 . p) (unless (distinct (apply append (map player-tiles p))) (error 'state0 (format "Precondition: distinct tiles for players ~a" p))) (apply state0 p))
   (: state0 (-> Player * State))
   (define (state0 . p)
   ...
   (define (state-sub-shares s bad-shares) (struct-copy state s (shares (shares-minus (state-shares s) bad-shares))))
   (: ext:*cs0 (-> String * State))
   (define (ext:*cs0 . names)
     (unless (distinct names) (error '*cs0 (format "Precondition: (distinct ~a)" names)))
     (unless (andmap (lambda ((s : String)) (<= (string-length s) 20)) names) (error 'cs0 (format "Precondition: strings with <=20 characters ~a" names)))
     (apply *cs0 names))
   (: *cs0 (-> String * State))
   (define (*cs0 . names)
     (let loop :
       State
       ((names : (Listof String) names) (tiles : (Listof Tile) ALL-TILES) (players : (Listof Player) '()))
       (cond
        ((empty? names) (state (make-board) (reverse players) tiles ALL-HOTELS banker-shares0 '()))
        (else
   ...
   (define (ext:*create-state b lp)
     ...
     (unless (distinct (apply append (board-tiles b) (map player-tiles lp))) (error 'create-state "precond"))
     (*create-state b lp))
   (: *create-state (-> Board (Listof Player) State))
   (define (*create-state board players)
     (define players-shares (for/list : (Listof Shares) ((p : Player (in-list players))) (player-shares p)))
     (define remaining-shares (for/fold : Shares ((remaining-shares banker-shares0)) ((s : Shares (in-list players-shares))) (shares-minus remaining-shares s)))
     (define remaining-hotels (for/list : (Listof Hotel) ((h : Hotel (in-list ALL-HOTELS)) #:when (= (size-of-hotel board h) 0)) h))
     (define remaining-tiles (remove* (apply append (board-tiles board) (map player-tiles players)) ALL-TILES))
   ...
   (define (ext:state-place-tile s tile (hotel #f))
     (unless (member tile (player-tiles (state-current-player s))) (error 'state-place-tile (format "Precondition: tile ~a belongs to player ~a" tile (state-current-player s))))
     (unless (not (eq? (what-kind-of-spot (state-board s) tile) 'IMPOSSIBLE)) (error 'state-place-tile (format "Precondition: impossible position for ~a on board" tile)))
     (let ((spot-type (what-kind-of-spot (state-board s) tile)))
       (when hotel (unless (memq spot-type '(FOUNDING MERGING)) (error 'state-place-tile (format "Precondition: expected founding or merging spot for ~a" tile))))
       (let ((b (state-board s)) (hotels (state-hotels s)))
         (unless (if (and (eq? spot-type 'FOUNDING) (pair? hotels)) (and hotel (member hotel hotels)) #t)
           (error 'state-place-tile (format "Precondition: if spot is FOUNDING and hotels are available, ~a must be one of them" hotel))))
       (unless (if (eq? spot-type 'MERGING) (and hotel (let-values (((w _) (merging-which (state-board s) tile))) (member hotel w))) #t)
   ...
   (define (state-place-tile s tile (hotel #f))
     ...
     (cond
      ((or (eq? SINGLETON spot) (eq? GROWING spot) (and (eq? FOUNDING spot) (not hotel)))
       (define new-board (if (eq? GROWING spot) (grow-hotel board tile) (place-tile board tile)))
       (struct-copy state s (board new-board) (tiles tiles-next) (players players-next)))
      ((eq? FOUNDING spot)
       (define t (struct-copy state s (hotels (remove hotel hotels)) (tiles tiles-next) (board (found-hotel board tile hotel))))
       (if (= (shares-available shares hotel) 0)
         (struct-copy state t (players players-next))
         (struct-copy state t (shares (shares-- shares hotel)) (players (cons (player-shares++ current hotel) others)))))
      ((eq? MERGING spot)
       (define-values (w l) (merging-which board tile))
       (define acquired (append (remove hotel w) l))
       (define next-state (struct-copy state s (board (merge-hotels board tile (or hotel (error 'hotel=#f)))) (hotels (append acquired hotels)) (tiles tiles-next) (players players-next)))
   ...
   (define ((state-distribute-bonus board) acquired-hotel s)
     ...
     (define owners-of-acquired-sorted (sort owners-of-acquired (lambda ((x : Player) (y : Player)) (> (selector x) (selector y)))))
     (cond
      ((empty? owners-of-acquired-sorted) s)
      (else
       (define majority-minority ((inst aux:partition Player Player) owners-of-acquired-sorted selector (lambda ((x : Player)) x)))
       ...
       (cond
        ...
        ((empty? minority) (define single-majority (first majority)) (struct-copy state s (players ((state-pay-out majority-bonus) single-majority players))))))))
   ...
   (define (ext:state-move-tile s t)
     (unless (member t (state-tiles s)) (error 'state-move-tile (format "Precondition: tile ~a must be a member of ~a" t (state-tiles s))))
     (state-move-tile s t))
   (: state-move-tile (-> State Tile State))
   (define (state-move-tile s t)
     (match-define (state _board players tiles _hotels _shares _bad) s)
     (struct-copy state s (players (cons (player-tile+ (first players) t) (rest players))) (tiles (remove t tiles))))
   (: state-next-turn (-> State State))
   (define (state-next-turn s) (define players (state-players s)) (struct-copy state s (players (append (rest players) (list (first players))))))
   (: state-remove-current-player (-> State State))
   (define (state-remove-current-player s) (define players (state-players s)) (struct-copy state s (players (rest players)) (bad (cons (first players) (state-bad s)))))
   (: state-eliminate (-> State (Listof Player) State))
   (define (state-eliminate s ep) (struct-copy state s (players (remove* ep (state-players s))) (bad (append ep (state-bad s)))))
   ...
   (define (ext:state-buy-shares s sh)
     ...
     (unless (affordable? (state-board s) sh (player-money (state-current-player s)))
       (error 'state-buy-shares (format "Precondition: player ~a cannot afford to buy ~a" (state-current-player s) sh)))
     (unless (let ((banker-s-shares (state-shares s))) (shares-available? banker-s-shares sh)) (error 'state-buy-shares (format "shares ~a are not available")))
   ...
   (define (state-buy-shares s sh)
     ...
     (struct-copy state s (players (cons (player-buy-shares (first players) sh board) (rest players))) (shares (for/fold : Shares ((s : Shares shares)) ((h : Hotel sh)) (shares-- s h)))))
   ...
   (define (ext:state-return-shares s decisions (board (state-board s)))
     (unless (= (length (state-players s)) (length decisions)) (error 'state-return-shares (format "Precondition: need same number of players and decisions")))
   ...
   (define (state-return-shares s decisions (board (state-board s)))
     (for/fold : State ((s : State s)) ((d : (List Player (Listof (List Hotel Boolean))) decisions)) (state-return-shares/player s (first d) (second d) board)))
   ...
   (define (state-return-shares/player s p p-s-decisions (board (state-board s)))
     ...
     (define new-players (for/list : (Listof Player) ((q : Player players)) (if (string=? (player-name q) the-name) (player-returns-shares q transfers board) q)))
   ...
   (define (shares-to-be-moved+their-value player-s-shares decisions)
     (for/fold
      ...
      (cond
       ((second d) shares)
       (else (define available (shares-available player-s-shares hotel)) (for/fold : Shares ((shares : Shares shares)) ((n : Natural (in-range available))) (shares++ shares hotel))))))
   ...
   (define (state-score s0)
     ...
     (define state/bonus (foldr (lambda ((h : Hotel) (s : State)) (if (= (size-of-hotel board h) 0) s (bonus h s))) s0 ALL-HOTELS))
     (define scores
       (for/list
        :
     ...
     (sort scores (lambda ((p : (List String Cash)) (q : (List String Cash))) (or (> (second p) (second q)) (and (= (second p) (second q)) (string<=? (first p) (first q)))))))
   (: shares->money (-> Shares Board Cash))
   (define (shares->money shares board)
     (assert
      (for/sum : Integer (((hotel n) (in-hash shares))) (define size (size-of-hotel board hotel)) (define price (price-per-share hotel size)) (if (and (> size 0) price) (* price n) 0))
   ...
   (define (state-final? s)
     ...
     (define-values
      ...
      (for/fold
       ...
       ((winner? : Boolean #f) (founded : (Listof Hotel) '()) (safe : (Listof Hotel) '()))
       ((h : Hotel ALL-HOTELS))
       (define s (size-of-hotel board h))
       (cond ((>= s SAFE#) (values (>= s FINAL#) (cons h founded) (cons h safe))) ((> s 0) (values winner? (cons h founded) safe)) (else (values winner? founded safe)))))

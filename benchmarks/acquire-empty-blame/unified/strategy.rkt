(module strategy typed/racket/shallow
  (#%module-begin
   (provide ordered-s random-s)
   ...
   (: pick-hotel (-> Board Tile Kind (Listof Hotel) (-> (Listof Hotel) Hotel) (-> (Listof Hotel) Hotel) (Option Hotel)))
   (define (pick-hotel b to-place kind available-hotels select-founding-hotel select-merger)
     (cond
      ((eq? FOUNDING kind) (and (cons? available-hotels) (select-founding-hotel available-hotels)))
      ((eq? MERGING kind) (define-values (w _) (merging-which b to-place)) (select-merger w))
      (else #f)))
   (: to-buy (-> Board Cash Shares Natural (Listof Hotel)))
   (define (to-buy b my-cash available-shares BUY-N)
     (let loop :
       (Listof Hotel)
       ((hotels : (Listof Hotel) ALL-HOTELS) (n : Natural 0) (to-buy : (Listof Hotel) '()) (cash : Cash my-cash) (as : Shares available-shares))
       (cond
        ((or (empty? hotels) (= BUY-N n)) (reverse to-buy))
        (else
         (displayln (eq-hash-code hotels))
         ...
         (cond
          ...
          (else
           ...
           (if (and price (<= price cash)) (if (and (<= (* 2 price) cash) (> available-h 1)) (list h h) (list h)) (loop (rest hotels) n to-buy cash as))))))))
   (: strategy/d (-> (-> (Listof Hotel) Hotel) (-> (Listof Hotel) Hotel) (-> Natural Natural) Strategy))
   (define (strategy/d choose-founding choose-merger choose-shares#)
     (lambda ((turn : (Instance Turn%)))
       ...
       (define tile-kind (for/or : (Option (List Tile Kind)) ((t : Tile (sort (get-field tiles turn) tile<=?))) (let ((s (what-kind-of-spot b t))) (and (not (eq? s IMPOSSIBLE)) (list t s)))))
       (cond
        (tile-kind
         ...
         (define shares (if (and (eq? FOUNDING kind) hotel (> (shares-available available-shares hotel) 0)) (shares-- available-shares hotel) available-shares))
        ...
        (else (define buy (to-buy b my-cash available-shares (choose-shares# nat-SHARES-PER-TURN#))) (values #f #f buy)))))
   (: id (All (A) (-> A A)))
   (define (id x) x)
   (: ordered-s Strategy)
   (define ordered-s (strategy/d (inst first Hotel Hotel) alphabetically-first (inst id Natural)))

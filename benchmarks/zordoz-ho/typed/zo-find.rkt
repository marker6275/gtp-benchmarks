...
(: zo-find-aux (-> zo (Listof zo) (-> String Boolean) Natural (U Natural #f) (Listof result)))
(define (zo-find-aux z hist found? i lim)
  ...
  (define results
    (cond
     ...
     [else
      ...
      (append-all (for/list : (Listof (Listof result)) ([z* : zo children])
                              (zo-find-aux z* hist* found? (add1 i) lim)))]))
  (if (and (found? title) (not (memq z (map result-zo results))))
